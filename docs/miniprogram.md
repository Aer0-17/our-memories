...
当前第一版复用项目已有的空间码、账号和密码登录，不依赖微信 AppID/Secret，也不会调用未实现的微信绑定接口。登录成功后，小程序会保存 access token 和 refresh token，并在 access token 过期时自动刷新。
...
新增回忆、图片上传、微信一键登录留到后续版本，避免在小程序审核前引入额外的上传域名和账号绑定逻辑。
