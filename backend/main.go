...
func main() {
	...
	fullBackupService, err := securebackup.NewService(securebackup.ServiceConfig{
		...
		ReplicaEnabled:                config.Get().FullBackupReplicaEnabled,
		ReplicaDirectory:              config.Get().FullBackupReplicaDir,
		ReplicaRetentionCount:         config.Get().FullBackupReplicaRetention,
